...
void implement_lock_init(uint64_t *context);
void implement_lock_acquire(uint64_t *context);
void implement_lock_release(uint64_t *context);

void emit_cond_init();
void emit_cond_wait();
void emit_cond_signal();

void implement_cond_init(uint64_t *context);
...
uint64_t SYSCALL_PT_CREATE	= 219;
uint64_t SYSCALL_PT_JOIN	= 220;
uint64_t SYSCALL_PT_EXIT	= 221;

uint64_t SYSCALL_SEM_INIT = 222;
uint64_t SYSCALL_SEM_WAIT = 223;
uint64_t SYSCALL_SEM_POST = 224;

uint64_t SYSCALL_LOCK_INIT = 225;
uint64_t SYSCALL_LOCK_ACQUIRE = 226;
uint64_t SYSCALL_LOCK_RELEASE = 227;
...
uint64_t get_virtual_address_of_page_end(uint64_t page);
...
uint64_t global_timer = 0; // global timer counter
...
// | 38 | tg leader		        | true if thread is tg leader (main process)
...
uint64_t is_leader(uint64_t* context) { return (uint64_t) (context + 38); }
uint64_t tg(uint64_t* context) { return (uint64_t) (context + 39); }
uint64_t tg_next(uint64_t* context) { return (uint64_t) (context + 40); }
...
uint64_t get_is_leader (uint64_t* context) { return *(context + 38); }
...
void set_is_leader(uint64_t* context, uint64_t is_leader) { *(context + 38) = is_leader; }
...
// cond variable
// +---+--------------------+
// | 0 | n_waiters			|      number of waiters
//   1   waiters        |      array of waiters
// +---+--------------------+
...
void set_cond_waiters (uint64_t *cond, uint64_t *waiters) { *(cond+1) = (uint64_t) waiters; }
...
uint64_t copy_registers(uint64_t* dst, uint64_t* src);
uint64_t copy_lo_pages(uint64_t* dst, uint64_t* src);
uint64_t copy_hi_pages(uint64_t* dst, uint64_t* src);
uint64_t copy_bounds(uint64_t* dst, uint64_t* src);
...
uint64_t* thread_groups = (uint64_t*) 0; // Array of singly linked lists of contexts in a thread group
uint64_t* thread_group_ctr = (uint64_t*) 0;	// Array of tg ids indicating number of threads per tg
...
uint64_t *used_cond = (uint64_t *)0;
uint64_t next_cond_id = 0;
// LOCK
...
void implement_exit(uint64_t* context) {
  ...
  //printf ("Exit code of thread %s is %lu\n", get_name(context), signed_int_exit_code);
	...
	if (parent != (uint64_t*) 0) {
		...
		if (get_status(parent) == STATUS_BLOCKED) {
			...
			if (wstatus_addr != (uint64_t) 0) {
				map_and_store(context, wstatus_addr, get_exit_code(context));
...
void emit_cond_init(){
  create_symbol_table_entry(GLOBAL_TABLE, string_copy("cond_init"),
    0, PROCEDURE, UINT64_T, 1, code_size);

  emit_load(REG_A0, REG_SP, 0); // variable
  emit_addi(REG_SP, REG_SP, WORDSIZE);

  emit_addi(REG_A7, REG_ZR, SYSCALL_COND_INIT);
...
void emit_cond_wait(){
  create_symbol_table_entry(GLOBAL_TABLE, string_copy("cond_wait"),
    0, PROCEDURE, UINT64_T, 2, code_size);

  emit_load(REG_A0, REG_SP, 0); // cond vari
  emit_addi(REG_SP, REG_SP, WORDSIZE);

  emit_load(REG_A1, REG_SP, 0); // lock var
  emit_addi(REG_SP, REG_SP, WORDSIZE);

  emit_addi(REG_A7, REG_ZR, SYSCALL_COND_WAIT);
...
void implement_cond_wait (uint64_t *context) {
  ...
  cond = used_cond + (cond_id * CONDENTRIES);
...
void emit_cond_signal(){
  create_symbol_table_entry(GLOBAL_TABLE, string_copy("cond_signal"),
    0, PROCEDURE, UINT64_T, 1, code_size);

  emit_load(REG_A0, REG_SP, 0); // cond vari
  emit_addi(REG_SP, REG_SP, WORDSIZE);

  emit_addi(REG_A7, REG_ZR, SYSCALL_COND_SIGNAL);
...
uint64_t try_brk(uint64_t* context, uint64_t new_program_break) {
  ...
  if (is_virtual_address_valid(new_program_break, WORDSIZE))
    if (is_address_between_stack_and_heap(context, new_program_break)) {
	  ...
	  current_thread = (uint64_t*) *(thread_groups + get_tg(context));
	  while (current_thread != (uint64_t*) 0) {
		set_program_break(current_thread, new_program_break);
		current_thread = get_tg_next(current_thread);
	  }
...
void implement_fork(uint64_t* context) {
	...
	copy_registers(child, context);
	copy_lo_pages(child, context);
	copy_hi_pages(child, context);
	copy_bounds(child, context);
...
void implement_wait(uint64_t* context) {
	...
	/*uint64_t start = get_heap_seg_start(context);
	for (uint64_t i = 0; i < 8 * WORDSIZE; i += WORDSIZE) {
		if (is_virtual_address_mapped (get_pt(context), start + i)) {
			printf ("Heap[%lu]: %lu\n", i / WORDSIZE, load_virtual_memory(get_pt(context), start + i));
		}
	}*/
}
...
void emit_pthread_create () {
	...
	emit_addi(REG_A7, REG_ZR, SYSCALL_PT_CREATE);
...
void implement_pthread_create (uint64_t* context) {
	...
	uint64_t sp_offset;
	uint64_t fp_offset;
	uint64_t n_threads_in_tg;

	//uint64_t print_log;

	thread = create_context(MY_CONTEXT, 0);

	/* Set process control attributes */
	set_parent_process(thread, context);
	set_pid(thread, next_pid);
	set_tg(thread, get_tg(context));
	set_nchildren(context, get_nchildren(context) + 1);
	next_pid = next_pid + 1;

	set_name(thread, string_copy(itoa(get_pid(thread), integer_buffer, 10, 0, 0)));		// Copy name

	/* Since copy is shallow, we share page table */
	set_pt(thread, get_pt(context));
	
	/* We start to copy the contexts */
	copy_registers(thread, context);
	copy_bounds (thread, context);

	/* Calculate thread's stack addresses */
	// We first assume a max stack size of 1 page (4096B)
	n_threads_in_tg = *(thread_group_ctr + get_tg(context));
	dst_hi_hi = get_page_of_virtual_address(HIGHESTVIRTUALADDRESS) - 2 * n_threads_in_tg;
	dst_lo_hi = dst_hi_hi - (get_highest_hi_page(context) - get_lowest_hi_page (context));

	/* Calculate the offset of the stack pointer and frame pointer based on the hi_hi_page */
	sp_offset = get_virtual_address_of_page_end(get_highest_hi_page (context)) - *(get_regs(thread) + REG_SP);
	fp_offset = get_virtual_address_of_page_end(get_highest_hi_page(context) ) - *(get_regs(thread) + REG_S0);
	
	set_lowest_hi_page(thread, dst_lo_hi);
	set_highest_hi_page(thread, dst_hi_hi);
	*(get_regs(thread) + REG_SP) = get_virtual_address_of_page_end(dst_hi_hi) - sp_offset;
	*(get_regs(thread) + REG_S0) = get_virtual_address_of_page_end(dst_hi_hi) - fp_offset;
	
	/* Stack must also be deeply copied but on the same pt */
	//print_log = 0;
	src_vaddr = get_virtual_address_of_page_start(get_lowest_hi_page(context));
	dst_vaddr = get_virtual_address_of_page_start(dst_lo_hi);
	while (get_page_of_virtual_address(src_vaddr) <= get_highest_hi_page(context)) {
		if (is_virtual_address_valid(src_vaddr, WORDSIZE) == 1) {
			if (is_virtual_address_mapped (get_pt(context), src_vaddr)) {
				/*if (load_virtual_memory(get_pt(context), src_vaddr) != (uint64_t) 0) {
					print_log = 1;
				}*/
				//if (print_log) printf("In context %s, page %lu address %lu contents are %lu\n", get_name(context),get_page_of_virtual_address(src_vaddr), src_vaddr, load_virtual_memory(get_pt(context), src_vaddr));
				map_and_store(thread, dst_vaddr, load_virtual_memory(get_pt(context), src_vaddr));
			}
		}
		
		src_vaddr = src_vaddr + WORDSIZE;
		dst_vaddr = dst_vaddr + WORDSIZE;
	}

	//printf("In context %s, current stack pointer is %lu\n", get_name(context), *(get_regs(context) + REG_SP));
	/* Increment tg thread count */
	*(thread_group_ctr + get_tg(context)) = *(thread_group_ctr + get_tg(context)) + 1;
	set_tg_next(thread, (uint64_t*) *(thread_groups + get_tg(thread)));
	*(thread_groups + get_tg(thread)) = (uint64_t) thread;

	*(get_regs(context) + REG_A0) = get_pid(thread);
	*(get_regs(thread) + REG_A0) = 0;

	set_pc(context, get_pc(context) + INSTRUCTIONSIZE);
	set_pc(thread, get_pc(context));
}
...
void emit_pthread_join() {
	...
	emit_load(REG_A1, REG_SP, 0);	// wstatus
	emit_addi(REG_SP, REG_SP, WORDSIZE);

	emit_addi(REG_A7, REG_ZR, SYSCALL_PT_JOIN);
...
void emit_pthread_exit() {
	...
	emit_load(REG_A0, REG_SP, 0);	// exit code
	emit_addi(REG_SP, REG_SP, WORDSIZE);

	emit_addi(REG_A7, REG_ZR, SYSCALL_PT_EXIT);
...
void implement_global_timer(uint64_t *context) {
  *(get_regs(context) + REG_A0) = global_timer;
...
void implement_sem_wait (uint64_t *context) {
  ...
  sem_id_address = *(get_regs (context) + REG_A0);
  sem_id = load_virtual_memory (get_pt (context), sem_id_address);
  sem = used_semaphores + (sem_id * SEMAPHOREENTRIES);
...
void implement_sem_post (uint64_t *context) {
  ...
  if (get_sem_n_waiters (sem) > 0) {	// If any waiters, we wake up a waiter
	waiter_idx = 0;

	while (*(get_sem_waiters(sem) + waiter_idx) == (uint64_t) -1) {
		waiter_idx = waiter_idx + 1;
	}

	set_blocked (find_context_by_id (*(get_sem_waiters (sem) + waiter_idx)), 0);
	*(get_sem_waiters (sem) + waiter_idx) = -1;
	set_sem_n_waiters (sem, get_sem_n_waiters (sem) - 1);
  } else {
  	set_sem_value (sem, get_sem_value (sem) + 1);
  }
...
void emit_lock_init() {
  ...
  emit_load(REG_A0, REG_SP, 0); // lock
  emit_addi(REG_SP, REG_SP, WORDSIZE);
...
uint64_t get_virtual_address_of_page_end(uint64_t page) {
	return get_virtual_address_of_page_start(page + 1) - WORDSIZE;
}
  ...
  if (redo) {
    ...
    *(registers + REG_A0) = *(values + (tc % MAX_REPLAY_LENGTH));
    ...
    if (record) {
      ...
      else if(*(registers + REG_A7) == SYSCALL_COND_SIGNAL){
        read_register(REG_A0);
      }
      else{
      if (*(registers + REG_A7) != SYSCALL_FORK) {
          if (*(registers + REG_A7) != SYSCALL_PT_CREATE) {
              read_register(REG_A0);
        if (*(registers + REG_A7) != SYSCALL_SEM_POST) {
        if (*(registers + REG_A7) != SYSCALL_SEM_WAIT) {
        if (*(registers + REG_A7) != SYSCALL_LOCK_INIT) {
        if (*(registers + REG_A7) != SYSCALL_LOCK_ACQUIRE) {
        if (*(registers + REG_A7) != SYSCALL_LOCK_RELEASE) {
              if (*(registers + REG_A7) != SYSCALL_EXIT) {
                if (*(registers + REG_A7) != SYSCALL_PT_EXIT) {
                  if (*(registers + REG_A7) != SYSCALL_BRK) {
                    if (*(registers + REG_A7) != SYSCALL_WAIT) {
                      if (*(registers + REG_A7) != SYSCALL_PT_JOIN) {
                          read_register(REG_A1);
...
uint64_t create_cond () {
  ...
  uint64_t i;
	...
	cond = used_cond + (cond_id * CONDENTRIES);
  set_cond_n_waiters (cond, 0);
	set_cond_waiters (cond, smalloc (sizeof (uint64_t) * 64)); // At most 64 waiters

	i = 0;
	while (i < 64) {
      *(get_cond_waiters (cond) + i) = -1;
	  i = i + 1;
	}
...
uint64_t copy_registers(uint64_t* dst, uint64_t* src) {
	...
	if (src == (uint64_t*) 0) {
		printf("Attempting to copy registers from a null context\n");
		return 1;
	}

	if (dst == (uint64_t*) 0) {
		printf("Attempting to copy registers into a null context\n");
		return 1;
	}

	i = 0;
	while (i < NUMBEROFREGISTERS) {
		*(get_regs(dst) + i) = *(get_regs(src) + i);		// Copy registers
		i = i + 1;
	}
...
uint64_t copy_lo_pages(uint64_t* dst, uint64_t* src) {
	...
	if (src == (uint64_t*) 0) {
		printf("Attempting to copy lo pages from a null context\n");
		return 1;
	}

	if (dst == (uint64_t*) 0) {
		printf("Attempting to copy lo pages into a null context\n");
		return 1;
	}

	page = get_lowest_lo_page(src);
	vaddr = page * PAGESIZE;
	while (get_page_of_virtual_address(vaddr) < get_highest_lo_page(src)) {
		//printf ("Copying address %lx (page %lu)\n", vaddr, get_page_of_virtual_address (vaddr));

		map_and_store (dst, vaddr, load_virtual_memory(get_pt(src), vaddr));
		vaddr = vaddr + WORDSIZE;
	}
...
uint64_t copy_hi_pages(uint64_t* dst, uint64_t* src) {
	...
	if (src == (uint64_t*) 0) {
		printf("Attempting to copy hi pages from a null context\n");
		return 1;
	}

	if (dst == (uint64_t*) 0) {
		printf("Attempting to copy hi pages into a null context\n");
		return 1;
	}

	page = get_lowest_hi_page(src);
	vaddr = page * PAGESIZE;
	while (get_page_of_virtual_address(vaddr) < get_highest_hi_page(src)) {
		map_and_store (dst, vaddr, load_virtual_memory(get_pt(src), vaddr));
		vaddr = vaddr + WORDSIZE;
	}
...
uint64_t copy_bounds(uint64_t* dst, uint64_t* src) {
	if (src == (uint64_t*) 0) {
		printf("Attempting to copy bounds from a null context\n");
		return 1;
	}

	if (dst == (uint64_t*) 0) {
		printf("Attempting to copy bounds into a null context\n");
		return 1;
	}

	set_lowest_lo_page(dst, get_lowest_lo_page(src));
	set_highest_lo_page(dst, get_highest_lo_page(src));
	set_lowest_hi_page(dst, get_lowest_hi_page(src));
	set_highest_hi_page(dst, get_highest_hi_page(src));

	/* Copy segment bounds */
	set_code_seg_start(dst, get_code_seg_start(src));
	set_code_seg_size(dst, get_code_seg_size(src));
	set_data_seg_start(dst, get_data_seg_start(src));
	set_data_seg_size(dst, get_data_seg_size(src));
	set_heap_seg_start(dst, get_heap_seg_start(src));
	set_program_break(dst, get_program_break(src));
...
uint64_t handle_system_call(uint64_t* context) {
  ...
  if (a7 == SYSCALL_BRK) {
    ...
    implement_wait(context);
  else if (a7 == SYSCALL_PT_CREATE)
    implement_pthread_create(context);
  else if (a7 == SYSCALL_PT_JOIN)
    implement_pthread_join(context);
  else if (a7 == SYSCALL_PT_EXIT) {
    ...
    implement_sem_post(context);
  else if (a7 == SYSCALL_LOCK_INIT)
    implement_lock_init(context);
  else if (a7 == SYSCALL_LOCK_ACQUIRE)
    implement_lock_acquire(context);
  else if (a7 == SYSCALL_LOCK_RELEASE)
    implement_lock_release(context);
  else if(a7 == SYSCALL_COND_INIT)
    implement_cond_init(context);
  else if(a7 == SYSCALL_COND_SIGNAL)
...
uint64_t selfie_run(uint64_t machine, uint64_t nproc) {
  ...
  set_tg(current_context, get_pid(current_context));
  set_tg_next(current_context, (uint64_t*) 0);
  *(thread_group_ctr + get_tg(current_context)) = *(thread_group_ctr + get_tg(current_context)) + 1;
  *(thread_groups + get_tg(current_context)) = (uint64_t) current_context;
